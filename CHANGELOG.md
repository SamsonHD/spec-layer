...
- See what changed. A drifted component page shows a collapsible **What changed in Figma** diff of the deterministic sections (Anatomy, Configuration, Variants, States, Tokens used, …) between the saved spec and the waiting re-extraction, so reviewers can see exactly what will be refreshed before applying the Update. Judgment prose is excluded because it is preserved.
- The plugin's per-selection doc-status chip no longer goes stale until restart: it re-checks automatically when the plugin regains focus, offers an explicit **Re-check**, and shows a transitional **Sent — apply in your inbox** state after **Update docs**.
